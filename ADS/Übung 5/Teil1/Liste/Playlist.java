...
public class Playlist {
	...
	public void hinzufuegenAmEnde(Musiktitel neuerMusiktitel) {
		if (root != null) {
			...
			// Schleife durchläuft die Listen-Elemente(Musikttitel), solgange
	...
	public void entferneMusiktitel(Musiktitel zuEntfernen) {
		...
		if (root.value == zuEntfernen) {
			...
			// Schleife durchläuft die Listen-Elemente(Musikttitel), solganege
	...
	public void leseAusDatei(File file) throws IOException {
		...
		while (eingabeZeile != null) {
			...
			// Erstelle einen Descriptor auf ein neues Musiktitel-Objekt,
			// welches dann hinten an die Queue(Liste) angefügt wird
	...
	public void bewegeTitelEinePositionWeiter(Musiktitel musiktitel) {
		...
		// Im Prinzip muss man nur Referenzen vertauschen. Darauf achten , das
		// beim kopieren Objektreferenzen ihre
		// Descriptoren nicht verlieren, da sonst der Garbage-Collector aktiv
		// wird. Dafür kann man eine Temporäre Variable
		// des gleichen Typs benutzen um den Descriptor zwischen zu speichern
		if (root != null) {
			if (middle.value != musiktitel) {
				// Schleife läuft solange durch bis der Musiktitel welcher
				// verschoben soll erreicht wird.
				...
				// Else behandelt wenn die oberen beiden Elemente der
				// Liste(Queu) verschoben werden.
				temp = root;
				root = root.nextElement;
